...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if len(password) < 8:
            self.stderr.write(self.style.ERROR('Пароль должен быть не менее 8 символов'))
            sys.exit(1)
        
        try:
            # Проверяем, существует ли уже пользователь
            if User.objects.filter(phone=phone).exists():
                self.stderr.write(self.style.ERROR(f'Пользователь с номером {phone} уже существует'))
                sys.exit(1)
