...
def post_edit(request, uuid):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Версия {version.version_number} успешно создана!"
            )
...
def post_versions(request, uuid):
    ...
    if request.method == "POST":
        ...
        if action == "set_current":
            post.set_current_version(version)
            messages.success(
                request, f"Версия {version.version_number} установлена как текущая"
            )
        elif action == "delete":
    ...
    return render(request, "post_versions.html", {"post": post, "versions": versions})
        ...
        try:
            ...
            intent = stripe.PaymentIntent.create(
                ...
                customer=(
                    ...
                    if hasattr(request.user, "stripe_customer_id")
                    else None
                ),
                metadata={
                    ...
                    "post_title": (
                        post.current_version.title if post.current_version else "Post"
                    ),
                },
            )
...
def post_purchase_confirm(request, uuid):
    ...
    try:
        ...
        messages.success(
            request, f'Доступ к посту "{post.current_version.title}" успешно получен!'
        )
...
def my_purchases(request):
    ...
    purchases = PostPurchase.objects.filter(user=request.user).select_related(
        "post", "post__current_version"
    )
