...
class UserModelTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            phone="+79374515429", password="testpass123"
        )
    ...
    def test_create_superuser(self):
        ...
        admin = User.objects.create_superuser(phone="+79999999999", password="admin123")
...
class PostModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(author=self.user, status="draft")
...
class PostCreateFormTest(TestCase):
    ...
    def test_invalid_form_short_content(self):
        ...
        form_data = {"title": "Тестовый пост", "content": "Короткий", "language": "ru"}
        form = PostCreateForm(data=form_data)
        self.assertFalse(form.is_valid())
...
class AuthViewsTest(TestCase):
    ...
    def test_login_view_post_valid(self):
        ...
        response = self.client.post(
            reverse("login"), {"username": "+79374515429", "password": "testpass123"}
        )
        self.assertEqual(response.status_code, 302)  # Redirect
...
class PostViewsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.version = self.post.create_version(
            title="Тестовый пост", content="Содержание тестового поста"
        )
        self.post.publish()
    ...
    def test_post_purchase_view(self, mock_stripe):
        ...
        mock_stripe.return_value = MagicMock(client_secret="test_secret_123")
...
class APITests(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(author=self.user)
        self.version = self.post.create_version(
            title="Тестовый пост", content="Содержание"
        )

    def test_post_access_check_free(self):
        """Тест API проверки доступа к бесплатному посту"""
        response = self.client.get(reverse("post_access_check", args=[self.post.uuid]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_post_access_check_subscription_with_auth(self):
        ...
        plan = SubscriptionPlan.objects.create(
            name="S", slug="s", price_monthly=1, price_yearly=10
        )
        self.post.access_type = "subscription"
        self.post.required_subscription = plan
        self.post.save()
...
class SecurityTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user2 = User.objects.create_user(
            phone="+79999999999", password="testpass123"
        )
        self.post = Post.objects.create(author=self.user1)
        self.version = self.post.create_version(
            title="Личный пост", content="Только для автора"
        )
...
class PerformanceTests(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            ...
            post.create_version(title=f"Пост {i}", content=f"Содержание поста {i}")
...
class AdditionalModelTests(TestCase):
    ...
    def test_postmedia_and_postpurchase_str(self):
        ...
        media = PostMedia.objects.create(
            post=post, uploaded_by=user, file="file.jpg", media_type="image"
        )
        self.assertIn("image for", str(media))
        purchase = PostPurchase.objects.create(
            user=user, post=post, amount=Decimal("5.00"), stripe_payment_intent_id="pi1"
        )
    ...
    def test_subscription_is_active_and_payment_str(self):
        ...
        plan = SubscriptionPlan.objects.create(
            name="P", slug="p", description="", price_monthly=1, price_yearly=10
        )
...
class FormExtraTests(TestCase):
    ...
    def test_settings_and_access_forms(self):
        form = PostSettingsForm(
            data={"is_featured": True, "allow_comments": False, "status": "draft"}
        )
        self.assertTrue(form.is_valid())
        plan = SubscriptionPlan.objects.create(
            name="X", slug="x", description="", price_monthly=1, price_yearly=10
        )
        form2 = PostAccessForm(
            data={
                "access_type": "subscription",
                "price": "",
                "required_subscription": plan.id,
            }
        )
...
class StripeViewsTest(TestCase):
    ...
    def test_create_checkout_no_price_id(self):
        self.client.login(username=self.user.phone, password="pw123")
        response = self.client.get(
            reverse("create_checkout_session", args=["t", "monthly"])
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_subscription_success_process(self, mock_sub, mock_sess):
        ...
        mock_sess.return_value = MagicMock(
            subscription="sub123", metadata={"plan_id": self.plan.id}
        )
...
class StripeWebhookTests(TestCase):
    ...
    def _post_event(self, event):
        ...
        return self.client.post(
            self.url, body, content_type="application/json", HTTP_STRIPE_SIGNATURE="sig"
        )
    ...
    def test_invoice_payment_succeeded(self, mock_construct):
        ...
        plan = SubscriptionPlan.objects.create(
            name="Y", slug="y", description="", price_monthly=1, price_yearly=10
        )
        sub = Subscription.objects.create(
            user=user,
            plan=plan,
    ...
    def test_invoice_payment_failed(self, mock_construct):
        plan = SubscriptionPlan.objects.create(
            name="Y2", slug="y2", description="", price_monthly=1, price_yearly=10
        )
        sub = Subscription.objects.create(
            user=User.objects.create_user(phone="+78888888888", password="pw"),
            plan=plan,
    ...
    def test_subscription_updated(self, mock_construct):
        plan = SubscriptionPlan.objects.create(
            name="Z", slug="z", description="", price_monthly=1, price_yearly=10
        )
        sub = Subscription.objects.create(
            user=User.objects.create_user(phone="+79999999998", password="pw"),
            plan=plan,
    ...
    def test_subscription_deleted(self, mock_construct):
        plan = SubscriptionPlan.objects.create(
            name="Z2", slug="z2", description="", price_monthly=1, price_yearly=10
        )
        sub = Subscription.objects.create(
            user=User.objects.create_user(phone="+79999999997", password="pw"),
            plan=plan,
        ...
        event = {
            ...
            "data": {"object": {"id": "sub4", "ended_at": 1}},
        }
        mock_construct.return_value = event
        res = self._post_event(event)
        sub.refresh_from_db()
