...
from .models import User, Post, PostVersion, PostMedia
...
class PostCreateForm(forms.Form):
    ...
    content = forms.CharField(
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 10, 'placeholder': 'Содержание поста'}),
        min_length=10
    )
...
class PostVersionForm(forms.ModelForm):
    ...
    change_summary = forms.CharField(
        max_length=200,
        required=False,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Что изменилось?'})
    )
    
    class Meta:
        model = PostVersion
        fields = ['title', 'content', 'summary', 'language', 'featured_image', 'meta_description']
        widgets = {
...
class PostAccessForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'access_type': forms.Select(attrs={'class': 'form-select', 'onchange': 'toggleAccessFields()'}),
