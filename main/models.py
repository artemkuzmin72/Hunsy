...
class Post(models.Model):
    ...
    purchased_by = models.ManyToManyField(
        User, through="PostPurchase", related_name="purchased_posts"
    )
    ...
    def create_version(
        self, title, content, summary="", meta_description="", language="ru"
    ):
...
class PostVersion(models.Model):
    ...
    change_summary = models.CharField(
        max_length=200, blank=True, help_text="Краткое описание изменений"
    )
    ...
    def get_previous_version(self):
        ...
        return (
            self.post.versions.filter(version_number__lt=self.version_number)
    ...
    def get_next_version(self):
        ...
        return (
            self.post.versions.filter(version_number__gt=self.version_number)
            .order_by("version_number")
            .first()
        )
...
class SubscriptionPlan(models.Model):
    ...
    is_active = models.BooleanField(default=True)
    sort_order = models.IntegerField(default=0)
    accessible_posts = models.ManyToManyField(
        Post, blank=True, related_name="subscription_plans"
    )
...
class Subscription(models.Model):
    ...
    user = models.ForeignKey(
        "main.User", on_delete=models.CASCADE, related_name="subscriptions"
    )
